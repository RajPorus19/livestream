...
from common.utils.loginTestUnit import login
...
class ChatTestUpdateReadDeletePermissions(TestCase):
    def setUp(self):
        ...
        self.staff = get_user_model().objects.create(
            username="staff", email="staff@mail.com", password="staff", is_staff=True
        )
        self.client = Client()
        self.message = ChatMsg.objects.create(
            message="my message", sender=self.user)
    ...
    def test_msg_update_from_user(self):
        self.client = login(self.client, self.user)
        response = self.client.put(
            self.chatMsgUrlDetail,
    ...
    def test_msg_update_from_otheruser(self):
        self.client = login(self.client, self.otheruser)
        response = self.client.put(
            self.chatMsgUrlDetail,
            json.dumps(self.data),
            content_type="application/json",
    ...
    def test_msg_update_from_staff(self):
        self.client = login(self.client, self.staff)
        response = self.client.put(
            self.chatMsgUrlDetail,
            json.dumps(self.data),
            content_type="application/json",
